...
В комплектной конфигурации выбран Performance с выводом в 1440p. При новом запуске с DLSS включается существующий денойзер для подготовки менее шумного сигнала; аккумуляция и денойзер остаются независимыми переключателями ImGui. Применение настроек сохраняет текущий выбор фильтров. Quality лучше сохраняет мелкие детали и требует больше времени на рендеринг; Performance снижает эти затраты и даёт более мягкое изображение.

Пример замеров в Bistro на RTX 4090, драйвер 610.88, Release, с включёнными аккумуляцией и денойзером:
...
Интеграция предоставляет **Super Resolution**, существующий денойзер остаётся отдельной опцией. Frame Generation и Ray Reconstruction не включены. Нужны поддерживаемая NVIDIA RTX и совместимый драйвер. Качество и время кадра зависят от сцены и выбранного режима; апскейлинг не гарантирует попиксельного совпадения с нативным рендером. Подробности — в [руководстве NVIDIA](https://github.com/NVIDIA-RTX/Streamline/blob/main/docs/ProgrammingGuideDLSS.md), условия распространения — в лицензии DLSS из SDK.
