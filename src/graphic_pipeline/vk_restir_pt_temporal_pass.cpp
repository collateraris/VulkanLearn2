...
void VulkanReSTIR_PT_TemporalPass::init(VulkanEngine* engine)
{
	_engine = engine;
	...
	{
		_engine->_renderPipelineManager.init_render_pipeline(_engine, EPipelineType::ReSTIR_PT_Temporal,
			[=](VkPipeline& pipeline, VkPipelineLayout& pipelineLayout) {
				ShaderEffect computeEffect;
				computeEffect.add_stage(_engine->_shaderCache.get_shader(VulkanEngine::shader_slang_path("restirPTTemporal.comp.slang.spv")), VK_SHADER_STAGE_COMPUTE_BIT);
...
void VulkanReSTIR_PT_TemporalPass::init_description_set_global_buffer()
{
...
Texture& VulkanReSTIR_PT_TemporalPass::get_tex(ETextureResourceNames name) const
{
