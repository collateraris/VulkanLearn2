...
void VulkanReSTIR_DI_InitPass::init(VulkanEngine* engine)
{
	_engine = engine;
	...
	{
		_engine->_renderPipelineManager.init_render_pipeline(_engine, EPipelineType::ReSTIR_DI_Init,
			[&](VkPipeline& pipeline, VkPipelineLayout& pipelineLayout) {
				ShaderEffect defaultEffect;
				uint32_t rayGenIndex = defaultEffect.add_stage(_engine->_shaderCache.get_shader(VulkanEngine::shader_slang_path("restir_di_start.rgen.slang.spv")), VK_SHADER_STAGE_RAYGEN_BIT_NV);
	...
	_rtSBTBuffer = VulkanRaytracerBuilder::create_SBTBuffer(engine, 2, 2, EPipelineType::ReSTIR_DI_Init,
		_rgenRegion,
		_missRegion,
		_hitRegion,
		_callRegion);
...
Texture& VulkanReSTIR_DI_InitPass::get_tex(ETextureResourceNames name) const
{
	return *_engine->get_engine_texture(name);
}


void VulkanReSTIR_DI_InitPass::init_description_set_global_buffer()
{
...
void VulkanReSTIR_DI_InitPass::init_tex()
{
	{
...
void VulkanReSTIR_DI_InitPass::draw(VulkanCommandBuffer* cmd, int current_frame_index)
{
	cmd->raytrace(&_rgenRegion, &_missRegion, &_hitRegion, &_callRegion, _imageExtent.width, _imageExtent.height, 1,
		[&](VkCommandBuffer cmd) {
			vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, _engine->_renderPipelineManager.get_pipeline(EPipelineType::ReSTIR_DI_Init));
			vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, _engine->_renderPipelineManager.get_pipelineLayout(EPipelineType::ReSTIR_DI_Init), 0,
			...
			vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, _engine->_renderPipelineManager.get_pipelineLayout(EPipelineType::ReSTIR_DI_Init), 1, 1, &_engine->get_engine_descriptor(currentGlobalUniformsDesc)->set, 0, nullptr);

			_rpDescrMan.bind_descriptor_set(cmd, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, _engine->_renderPipelineManager.get_pipelineLayout(EPipelineType::ReSTIR_DI_Init), 2);
		});
}
