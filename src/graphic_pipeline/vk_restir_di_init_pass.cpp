...
void VulkanReSTIR_DI_InitPass::draw(rhi::CommandList& cmd, int frameSlot)
{
	const auto pipeline = _engine->_rhi.pipeline(
		_engine->_renderPipelineManager.get_pipeline(EPipelineType::ReSTIR_DI_Init),
		_engine->_renderPipelineManager.get_pipelineLayout(EPipelineType::ReSTIR_DI_Init),
