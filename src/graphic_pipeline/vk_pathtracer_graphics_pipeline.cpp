#include "vk_pathtracer_graphics_pipeline.h"
...
void VulkanPathTracerGraphicsPipeline::init(VulkanEngine* engine)
{
	_engine = engine;
	...
	{
		_engine->_renderPipelineManager.init_render_pipeline(_engine, EPipelineType::PathTracer,
			[&](VkPipeline& pipeline, VkPipelineLayout& pipelineLayout) {
				...
				defaultEffect.add_stage(_engine->_shaderCache.get_shader(VulkanEngine::shader_path("pathtracer.frag.spv")), VK_SHADER_STAGE_FRAGMENT_BIT);
				...
				VkPipeline meshPipeline = pipelineBuilder.build_graphic_pipeline(_engine->_device,
					_engine->_renderPassManager.get_render_pass(ERenderPassType::PathtracerOutput)->get_render_pass());
...
void VulkanPathTracerGraphicsPipeline::copy_global_uniform_data(VulkanPathTracerGraphicsPipeline::SGlobalCamera& camData, int current_frame_index)
{
	_engine->map_buffer(_engine->_allocator, _globalCameraBuffer[current_frame_index]._allocation, [&](void*& data) {
		memcpy(data, &camData, sizeof(VulkanPathTracerGraphicsPipeline::SGlobalCamera));
		});
}
...
void VulkanPathTracerGraphicsPipeline::draw(VulkanCommandBuffer* cmd, int current_frame_index)
{
	...
	VkRenderPassBeginInfo rpInfo = vkinit::renderpass_begin_info(_engine->_renderPassManager.get_render_pass(ERenderPassType::PathtracerOutput)->get_render_pass(), VkExtent2D(_texExtent.width, _texExtent.height), _framebuffer);
	...
	cmd->draw_mesh_tasks_indirect(_indirectBuffer, indirect_offset, _objectsSize, draw_stride,
		[&](VkCommandBuffer cmd) {
			vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, _engine->_renderPipelineManager.get_pipeline(EPipelineType::PathTracer));

			vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, _engine->_renderPipelineManager.get_pipelineLayout(EPipelineType::PathTracer), 0, 1, &_engine->get_engine_descriptor(EDescriptorResourceNames::Bindless_Scene)->set, 0, nullptr);
			vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, _engine->_renderPipelineManager.get_pipelineLayout(EPipelineType::PathTracer), 1, 1, &_globalDescSet[current_frame_index], 0, nullptr);
		});
...
void VulkanPathTracerGraphicsPipeline::barrier_for_reading(VulkanCommandBuffer* cmd)
{
	std::array<VkImageMemoryBarrier, 1> barriers =
	{
		vkinit::image_barrier(get_output().image._image, VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT,  VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_ASPECT_COLOR_BIT),
	};

	vkCmdPipelineBarrier(cmd->get_cmd(), VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, 0, 0, 0, barriers.size(), barriers.data());
}

void VulkanPathTracerGraphicsPipeline::barrier_for_writing(VulkanCommandBuffer* cmd)
{
	std::array<VkImageMemoryBarrier, 1> barriers =
	{
		vkinit::image_barrier(get_output().image._image, VK_ACCESS_SHADER_READ_BIT, VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,  VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,  VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_ASPECT_COLOR_BIT),
	};

	vkCmdPipelineBarrier(cmd->get_cmd(), VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0, 0, 0, 0, 0, barriers.size(), barriers.data());
}

const Texture& VulkanPathTracerGraphicsPipeline::get_output() const
{
	return *_engine->get_engine_texture(ETextureResourceNames::PATHTRACER_OUTPUT);
}
...
void VulkanPathTracerGraphicsPipeline::init_tex()
{
...

	{
		...
		texBuilder.start()
			.make_img_info(_colorFormat, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, _texExtent)
			...
			.create_engine_texture(ETextureResourceNames::PATHTRACER_OUTPUT);
	}

	{
		...
		texBuilder.start()
			...
			.create_engine_texture(ETextureResourceNames::PATHTRACER_DEPTH_BUFFER);
	}
}

void VulkanPathTracerGraphicsPipeline::init_render_pass()
{
	...
	default_rp.color_attachments[0] = _engine->get_engine_texture(ETextureResourceNames::PATHTRACER_OUTPUT);
	default_rp.depth_stencil = _engine->get_engine_texture(ETextureResourceNames::PATHTRACER_DEPTH_BUFFER);
	...
	_engine->_renderPassManager.get_render_pass(ERenderPassType::PathtracerOutput)->init(_engine, default_rp);

	{
		...
		fb_info.renderPass = _engine->_renderPassManager.get_render_pass(ERenderPassType::PathtracerOutput)->get_render_pass();
		fb_info.width = _texExtent.width;
		fb_info.height = _texExtent.height;
		fb_info.layers = 1;

		std::array<VkImageView, 2> attachments;
		attachments[0] = _engine->get_engine_texture(ETextureResourceNames::PATHTRACER_OUTPUT)->imageView;
		attachments[1] = _engine->get_engine_texture(ETextureResourceNames::PATHTRACER_DEPTH_BUFFER)->imageView;
...
void VulkanPathTracerGraphicsPipeline::init_scene_buffer(const std::vector<RenderObject>& renderables)
{

	_objectsSize = renderables.size();
	_objectBuffer = _engine->create_cpu_to_gpu_buffer(sizeof(VulkanPathTracerGraphicsPipeline::ObjectData) * renderables.size(), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);

	for (int i = 0; i < FRAME_OVERLAP; i++)
	{
		_globalCameraBuffer[i] = _engine->create_cpu_to_gpu_buffer(sizeof(VulkanPathTracerGraphicsPipeline::SGlobalCamera), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
	...
	_engine->map_buffer(_engine->_allocator, _objectBuffer._allocation, [&](void*& data) {
		VulkanPathTracerGraphicsPipeline::ObjectData* objectSSBO = (VulkanPathTracerGraphicsPipeline::ObjectData*)data;
